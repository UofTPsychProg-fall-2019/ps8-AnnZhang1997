...
tbl_clean <- rename(tbl_clean, id=session_id, race=raceomb_002, bias=D_biep.White_Good_all, 
                    rt=Mn_RT_all_3467, edu=edu_14, pol=politicalid_7, state=STATE, att=att_7, 
                    temp_b=tblacks_0to10, temp_w=twhites_0to10, lab=labels)
...
# first find the improperly coded values
tbl_clean %>% 
  distinct(tbl_clean$gender)
tbl_clean %>%
  distinct(tbl_clean$state)
...
gender_count <- gender_count %>% filter(!gender=="Missing") %>% arrange(desc(n))
...
tbl_clean$gender4 <- tbl_clean$gender %>% recode("[1]"='male', "[2]"='female', "[3]"='queer/nonconforming', .default='other')
...
tbl_clean$edu7 <- recode_factor(as.factor(tbl_clean$edu),
                                '1'='no highschool','2'='no highschool',
                                '3'='some highschool','4'='highschool graduate',
                                '5'='some college','6'='postsecondary degree',
                                '7'='postsecondary degree','8'='postsecondary degree',
                                '11'='advanced degree','10'='advanced degree',
                                '9'='masters (MA & MB)','14'='masters (MA & MB)',
                                '12'='advanced degree','13'='advanced degree')

# mutating variables ---------------------------------------------  
# rewrite the above recoding steps so that they both occur within a single call of the mutate function
tbl_clean <- mutate(tbl_clean, gender4=recode(gender,"[1]"='male', "[2]"='female', 
                                              "[3]"='queer/nonconforming', .default='other'),
                    edu7=recode(as.factor(edu),
                                '1'='no highschool','2'='no highschool',
                                '3'='some highschool','4'='highschool graduate',
                                '5'='some college','6'='postsecondary degree',
                                '7'='postsecondary degree','8'='postsecondary degree',
...
# white men
wm <- filter(tbl_clean, race == 6, gender4 == 'male')
print(mean(wm$bias))

# white women
ww <- filter(tbl_clean, race == 6, gender4 == 'female')
print(mean(ww$bias))

# advanced degree holders who are men
adm <- filter(tbl_clean, edu7 == 'advanced degree', gender4 == 'male')
print(mean(adm$bias))

# high school graduates who are men
hgm <- filter(tbl_clean, edu7 == 'highschool graduate', gender4 == 'male')
print(mean(hgm$bias))
